...
// ITTAGE prediction-source tracking enum (was in the UCP_ISCA24 instruction.h)
...
class my_predictor
{
  ...
  int8_t spec_USE_ALT_ON_NA;		// "Use alternate prediction on weak predictions":  a 4-bit counter  to determine whether the newly allocated entries should be considered as  valid or not for delivering  the prediction
...
#define PRINTCHAR
  ...
  prediction_source get_last_source_btb() {
    return last_pred_source;
  }

  int get_last_btb_pointer_for_pred() {
    return last_pointer_for_pred;
  }
