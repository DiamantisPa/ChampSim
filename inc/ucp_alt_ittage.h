// Alt-Ind for the UCP port: 4KB ITTAGE (class alt_ittage), taken verbatim from the
// UCP_ISCA24 artifact (btb/twolevel_btb_1Ke_32Ke_gshare_ittage/alt_ittage.h).
// Self-contained; speculative_begin()/speculative_end() checkpoint/restore for
// alternate-path walks.  Included ONLY by src/ooo_cpu.cc, AFTER ucp_alt_tage.h --
// the leading #undefs clear that header's same-named config macros, and the
// trailing #undefs keep this header's local too.
#ifndef UCP_ALT_ITTAGE_H
#define UCP_ALT_ITTAGE_H
#undef NHIST
#undef SHARINGTABLES
#undef INITHISTLENGTH
#undef IUM
#undef LOGG
#undef TBITS
#undef MINHIST
...
#undef STEP1
#undef STEP2
#define H2P_INDIRECT_SIZE_ITTAGE 4 // 4KB flavor (paper Sec. VI-B: 12.95KB total)

// the artifact gets this enum from its own instruction.h; ours does not have it
#ifndef UCP_PREDICTION_SOURCE_ENUM
#define UCP_PREDICTION_SOURCE_ENUM
enum prediction_source { ALT_BANK, HIT_BANK, BIMODAL, LOOP, SAT_PRED, NONE };
#endif
/*
Code has been largely inspired  by the tagged PPM predictor simulator from Pierre Michaud, the OGEHL predictor simulator from by Andr� Seznec, the TAGE
predictor simulator from  Andr� Seznec and Pierre Michaud, the L-TAGE simulator by Andre Seznec
*/
...
#if (H2P_INDIRECT_SIZE_ITTAGE == 64)
#define LOGG 12
#endif
#if (H2P_INDIRECT_SIZE_ITTAGE == 32)
#define LOGG 11
#endif
#if (H2P_INDIRECT_SIZE_ITTAGE == 16)
#define LOGG 10
#endif
#if (H2P_INDIRECT_SIZE_ITTAGE == 8)
#define LOGG 9
#endif
#if (H2P_INDIRECT_SIZE_ITTAGE == 4)
#define LOGG 8
#endif
...
class alt_ittage
{
  ...
  {
  public:
    uint64_t tag, spec_tag;
    uint64_t pred, spec_pred;
    specentry()
    { // nothing
    }
  };
  ...
  int8_t USE_ALT_ON_NA; // "Use alternate prediction on weak predictions":  a 4-bit counter  to determine whether the newly allocated entries should be
  // considered as  valid or not for delivering  the prediction
  ...
  int8_t spec_USE_ALT_ON_NA; // "Use alternate prediction on weak predictions":  a 4-bit counter  to determine whether the newly allocated entries should be
  // considered as  valid or not for delivering  the prediction
  ...
  // for management at fetch time
  uint64_t spec_Fetch_ptghist;
  folded_history spec_Fetch_ch_i[NHIST + 1];    // utility for computing TAGE indices
  folded_history spec_Fetch_ch_t[2][NHIST + 1]; // utility for computing TAGE tags

  // for management at retire time
  uint64_t spec_Retire_ptghist;
  folded_history spec_Retire_ch_i[NHIST + 1];    // utility for computing TAGE indices
  folded_history spec_Retire_ch_t[2][NHIST + 1]; // utility for computing TAGE tags

  prediction_source spec_last_pred_source;
  int spec_last_pointer_for_pred;

  // predictor tables
  gentry* spec_gtable[NHIST + 1]; // ITTAGE tables (T0 has no tags other tables are tagged)
  regionentry* spec_rtable;       // Target region tables

  int spec_TB[NHIST + 1];        // tag width for the different tagged tables
  int spec_logg[NHIST + 1];      // log of number entries of the different tagged tables
  int spec_GI[NHIST + 1];        // indexes to the different tables are computed only once
  uint64_t spec_GTAG[NHIST + 1]; // tags for the different tables are computed only once

  uint64_t spec_pred_taken; // prediction
  uint64_t spec_alttaken;   // alternate  TAGEprediction
  uint64_t spec_tage_pred;  // TAGE prediction
  int spec_HitBank;         // longest matching bank
  int spec_AltBank;         // alternate matching bank
  uint64_t spec_LongestMatchPred;

  int spec_Seed; // for the pseudo-random number generator

  // for the IUM
  int spec_PtIumRetire;
  int spec_PtIumFetch;
  specentry* spec_IUMPred;

public:
  alt_ittage(void)
  {
    // cout << "4KB ITTAGE " << endl;
    USE_ALT_ON_NA = 0;
    Seed = 0;
    PtIumRetire = 0;
    PtIumFetch = 0;
...
#define STEP2 11
    ...
    spec_gtable[0] = new gentry[1 << logg[0]];
    spec_gtable[1] = new gentry[1 << logg[1]];
    spec_gtable[STEP1] = new gentry[1 << logg[STEP1]];
    spec_gtable[STEP2] = new gentry[1 << logg[STEP2]];
    for (int i = 1; i <= STEP1 - 1; i++) {
      spec_gtable[i] = spec_gtable[1];
    }

    for (int i = STEP1; i <= STEP2 - 1; i++) {
      spec_gtable[i] = spec_gtable[STEP1];
    }

    for (int i = STEP2; i <= 15; i++) {
      spec_gtable[i] = spec_gtable[STEP2];
    }
    ...
    spec_IUMPred = new specentry[1 << LOGSPEC];
#ifndef SHARINGTABLES
...
#define PRINTCHAR
    ...
    int NBENTRY = 0;
    int STORAGESIZE = 0;
    fprintf(stdout, "history lengths for alternate ITTAGE Predictor: ");
...
#endif
    fprintf(stdout, "Alternate ITTAGE tables: %d bytes;", STORAGESIZE / 8);
    fprintf(stdout, "Alternate Region table: %d bytes;", 15 * 128 / 8);
  ...
  prediction_source get_last_source_btb() { return last_pred_source; }

  int get_last_btb_pointer_for_pred() { return last_pointer_for_pred; }
  ...
  uint64_t predict_brindirect(uint64_t pc)
  {
    ...
    assert(last_pred_source == ALT_BANK || last_pred_source == HIT_BANK); // can be either altbank or hitbank
  ...
  void update_brindirect(uint64_t pc, uint8_t branch_type, uint8_t taken, uint64_t target)
  {
    ...
    if ((branch_type == BRANCH_INDIRECT) || (branch_type == BRANCH_INDIRECT_CALL)) {
      ...
      // Recompute the prediction by the ITTAGE predictor: on an effective implementation one would try to avoid this recomputation by propagating the
      // prediction with the branch instruction
      for (int i = 0; i <= NHIST; i++) {
  ...
  {
    spec_USE_ALT_ON_NA = USE_ALT_ON_NA;

    for (int i = 0; i < HISTBUFFERLENGTH; i++) {
      spec_ghist[i] = ghist[i];
    }

    spec_Fetch_ptghist = Fetch_ptghist;

    for (int i = 0; i < (1 << logg[0]); i++) {
      spec_gtable[0][i].ctr = gtable[0][i].ctr;
      spec_gtable[0][i].tag = gtable[0][i].tag;
      spec_gtable[0][i].target = gtable[0][i].target;
      spec_gtable[0][i].u = gtable[0][i].u;
    }

    // gtable[1] = new gentry[1 << logg[1]];
    for (int i = 0; i < (1 << logg[1]); i++) {
      spec_gtable[1][i].ctr = gtable[1][i].ctr;
      spec_gtable[1][i].tag = gtable[1][i].tag;
      spec_gtable[1][i].target = gtable[1][i].target;
      spec_gtable[1][i].u = gtable[1][i].u;
    }

    for (int i = 0; i < (1 << logg[STEP1]); i++) {
      spec_gtable[STEP1][i].ctr = gtable[STEP1][i].ctr;
      spec_gtable[STEP1][i].tag = gtable[STEP1][i].tag;
      spec_gtable[STEP1][i].target = gtable[STEP1][i].target;
      spec_gtable[STEP1][i].u = gtable[STEP1][i].u;
    }

    // gtable[1] = new gentry[1 << logg[1]];
    for (int i = 0; i < (1 << logg[STEP2]); i++) {
      spec_gtable[STEP2][i].ctr = gtable[STEP2][i].ctr;
      spec_gtable[STEP2][i].tag = gtable[STEP2][i].tag;
      spec_gtable[STEP2][i].target = gtable[STEP2][i].target;
      spec_gtable[STEP2][i].u = gtable[STEP2][i].u;
    }

    for (int i = 1; i <= STEP1 - 1; i++) {
      for (int j = 0; j < (1 << logg[1]); j++) {
        spec_gtable[i][j].ctr = gtable[i][j].ctr;
        spec_gtable[i][j].tag = gtable[i][j].tag;
        spec_gtable[i][j].target = gtable[i][j].target;
        spec_gtable[i][j].u = gtable[i][j].u;
      }
    }

    for (int i = STEP1; i <= STEP2 - 1; i++) {
      for (int j = 0; j < (1 << logg[STEP1]); j++) {
        spec_gtable[i][j].ctr = gtable[i][j].ctr;
        spec_gtable[i][j].tag = gtable[i][j].tag;
        spec_gtable[i][j].target = gtable[i][j].target;
        spec_gtable[i][j].u = gtable[i][j].u;
    ...
    for (int i = 0; i < NHIST + 1; i++) {
      ...
      spec_Fetch_ch_i[i].CLENGTH = Fetch_ch_i[i].CLENGTH;
      spec_Fetch_ch_i[i].comp = Fetch_ch_i[i].comp;
      spec_Fetch_ch_i[i].OLENGTH = Fetch_ch_i[i].OLENGTH;
      spec_Fetch_ch_i[i].OUTPOINT = Fetch_ch_i[i].OUTPOINT;

      spec_Retire_ch_t[1][i].CLENGTH = Retire_ch_t[1][i].CLENGTH;
      spec_Retire_ch_t[1][i].comp = Retire_ch_t[1][i].comp;
      spec_Retire_ch_t[1][i].OLENGTH = Retire_ch_t[1][i].OLENGTH;
      spec_Retire_ch_t[1][i].OUTPOINT = Retire_ch_t[1][i].OUTPOINT;

      spec_Retire_ch_t[2][i].CLENGTH = Retire_ch_t[2][i].CLENGTH;
      spec_Retire_ch_t[2][i].comp = Retire_ch_t[2][i].comp;
      spec_Retire_ch_t[2][i].OLENGTH = Retire_ch_t[2][i].OLENGTH;
      spec_Retire_ch_t[2][i].OUTPOINT = Retire_ch_t[2][i].OUTPOINT;
    }

    spec_Retire_ptghist = Retire_ptghist;

    spec_last_pred_source = last_pred_source;
    spec_last_pointer_for_pred = last_pointer_for_pred;

    spec_rtable->region = rtable->region;
    spec_rtable->u = rtable->u;

    spec_pred_taken = pred_taken;
    spec_alttaken = alttaken;
    spec_tage_pred = tage_pred;
    ...
    // for the IUM
    spec_PtIumRetire = PtIumRetire;
    spec_PtIumFetch = PtIumFetch;

    for (auto i = 0; i < (1 << LOGSPEC); i++) {
      spec_IUMPred[i].tag = IUMPred[i].tag;
      spec_IUMPred[i].pred = IUMPred[i].pred;
    }
  }

  void speculative_end()
  {
    USE_ALT_ON_NA = spec_USE_ALT_ON_NA;

    for (int i = 0; i < HISTBUFFERLENGTH; i++)
      ghist[i] = spec_ghist[i];

    Fetch_ptghist = spec_Fetch_ptghist;

    // gtable[0] = new gentry[1 << logg[0]];
    for (int i = 0; i < (1 << logg[0]); i++) {
      gtable[0][i].ctr = spec_gtable[0][i].ctr;
      gtable[0][i].tag = spec_gtable[0][i].tag;
      gtable[0][i].target = spec_gtable[0][i].target;
      gtable[0][i].u = spec_gtable[0][i].u;
    }
    ...
    for (int i = 0; i < (1 << logg[1]); i++) {
      ...
      gtable[1][i].target = spec_gtable[1][i].target;
      gtable[1][i].u = spec_gtable[1][i].u;
    }

    for (int i = 0; i < (1 << logg[STEP1]); i++) {
      gtable[STEP1][i].ctr = spec_gtable[STEP1][i].ctr;
      gtable[STEP1][i].tag = spec_gtable[STEP1][i].tag;
      gtable[STEP1][i].target = spec_gtable[STEP1][i].target;
      gtable[STEP1][i].u = spec_gtable[STEP1][i].u;
    }

    // gtable[1] = new gentry[1 << logg[1]];
    for (int i = 0; i < (1 << logg[STEP2]); i++) {
      gtable[STEP2][i].ctr = spec_gtable[STEP2][i].ctr;
      gtable[STEP2][i].tag = spec_gtable[STEP2][i].tag;
      gtable[STEP2][i].target = spec_gtable[STEP2][i].target;
      gtable[STEP2][i].u = spec_gtable[STEP2][i].u;
    }
    ...
    for (int i = 1; i <= STEP1 - 1; i++) {
      for (int j = 0; j < (1 << logg[1]); j++) {
        ...
        gtable[i][j].target = spec_gtable[i][j].target;
    ...
    for (int i = 0; i < NHIST + 1; i++) {
      ...
      Fetch_ch_i[i].CLENGTH = spec_Fetch_ch_i[i].CLENGTH;
      Fetch_ch_i[i].comp = spec_Fetch_ch_i[i].comp;
      Fetch_ch_i[i].OLENGTH = spec_Fetch_ch_i[i].OLENGTH;
      Fetch_ch_i[i].OUTPOINT = spec_Fetch_ch_i[i].OUTPOINT;

      Retire_ch_t[1][i].CLENGTH = spec_Retire_ch_t[1][i].CLENGTH;
      Retire_ch_t[1][i].comp = spec_Retire_ch_t[1][i].comp;
      Retire_ch_t[1][i].OLENGTH = spec_Retire_ch_t[1][i].OLENGTH;
      Retire_ch_t[1][i].OUTPOINT = spec_Retire_ch_t[1][i].OUTPOINT;

      Retire_ch_t[2][i].CLENGTH = spec_Retire_ch_t[2][i].CLENGTH;
      Retire_ch_t[2][i].comp = spec_Retire_ch_t[2][i].comp;
      Retire_ch_t[2][i].OLENGTH = spec_Retire_ch_t[2][i].OLENGTH;
      Retire_ch_t[2][i].OUTPOINT = spec_Retire_ch_t[2][i].OUTPOINT;
    }
    Retire_ptghist = spec_Retire_ptghist;

    last_pred_source = spec_last_pred_source;
    last_pointer_for_pred = spec_last_pointer_for_pred;

    rtable->region = spec_rtable->region;
    rtable->u = spec_rtable->u;

    pred_taken = spec_pred_taken;
    alttaken = spec_alttaken;
    tage_pred = spec_tage_pred;
    ...
    // for the IUM
    PtIumRetire = spec_PtIumRetire;
    PtIumFetch = spec_PtIumFetch;

    for (auto i = 0; i < (1 << LOGSPEC); i++) {
      IUMPred[i].tag = spec_IUMPred[i].tag;
      IUMPred[i].pred = spec_IUMPred[i].pred;
    }
  }
};
// keep this header's configuration macros from leaking into the including TU
#undef NHIST
#undef SHARINGTABLES
#undef INITHISTLENGTH
#undef IUM
...
#undef STEP1
#undef STEP2
#undef H2P_INDIRECT_SIZE_ITTAGE
#endif // UCP_ALT_ITTAGE_H
